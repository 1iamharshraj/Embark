  ...
  const filterSlot = (
    <form className="flex flex-wrap items-center gap-2">
      ...
      <div className="flex gap-2">
        <button
          type="submit"
          className="inline-flex items-center justify-center rounded-full font-semibold bg-orangeDeep text-white px-5 py-2 text-sm hover:bg-[#1740A8] transition"
        >
          Filter
        </button>
        <Link
          href="/admin/audit-logs"
          className="inline-flex items-center justify-center rounded-full font-semibold bg-cream text-charcoal border border-charcoal/15 px-5 py-2 text-sm hover:bg-orange/10 transition"
        >
          Reset
        </Link>
      </div>
    ...
    <>
      ...
      <div className="flex gap-2">
        <Link
          ...
          className={`inline-flex items-center justify-center rounded-full font-semibold px-4 py-2 text-sm transition ${
            page <= 1
              ? "bg-cream text-inkSoft border border-charcoal/10 pointer-events-none"
              : "bg-white border border-charcoal/8 text-charcoal hover:border-orange/40"
          }`}
        >
          Previous
        </Link>
  ...
  return (
    <>
      <AdminHeader
        eyebrow="Compliance"
        title="Audit logs"
        description="Trace actions taken across the platform by admin users."
        backHref="/admin"
      ...
      <AdminDataTable
        ...
        description="Search and filter activity records by actor, action, resource or date range."
        count={total}
        filterSlot={filterSlot}
        pagination={totalPages > 1 ? pagination : undefined}
        empty={
          logs.length === 0 && (
            <div className="p-8 text-center text-inkSoft">No audit logs found.</div>
          )
        }
      >
