// Static stream playbook content used as fallback / enrichment for the Next.js pages.
// The Prisma Playbook model remains the source of truth; these records supplement
// missing DB entries and drive the 3D shelf visuals.
...
export interface StaticPlanPhase {
  phase: string;
  detail: string;
}
...
export interface StaticPlaybook {
  slug: string;
  name: string;
...
export const STREAM_PLAYBOOKS: StaticPlaybook[] = [
  {
...
export const SHELF_BOOKS: ShelfBook[] = [
  {
    ...
    spineLabel: "Consulting",
    subtitle: "the case-cracker's route",
    theme: "dark",
    coverClass: "cv-soon",
    size: "h-s",
    soon: true,
  ...
  },
  {
    ...
    name: "General Management",
    displayName: "General\nManage­ment",
    spineLabel: "Gen Management",
    subtitle: "",
    theme: "dark",
    coverClass: "cv-gm",
    size: "h-l",
    pillLabel: "Gen Management",
  },
  {
    slug: "marketing",
    name: "Marketing",
    displayName: "Own\nthe\nShelf",
    spineLabel: "Marketing",
    subtitle: "the marketing route",
    theme: "orange",
    coverClass: "cv-mkt",
    size: "h-m",
    pillLabel: "Marketing",
...
export function getStaticPlaybook(slug: string): StaticPlaybook | undefined {
  return STREAM_PLAYBOOKS.find((p) => p.slug === slug);
}
